...
try:
    ...
    if v_func_name[:4] == "sub_":
        v_func_name =  "vfunc_" + str(v_func_name)[4:] # str(hex(v_func_addr- SegStart(v_func_addr))) the name will be the offset from the beginning of the segment
    ...
    if struct_id != idc.BADADDR: # checks if the struct creation succeeded
        ...
        while vtable_func_value != 0:
            ...
            if v_func_name[:4] == "sub_":
                v_func_name =  "vfunc_" + str(v_func_name)[4:] # str(hex(v_func_addr- SegStart(v_func_addr))) the name will be the offset from the beginning of the segment
                idaapi.set_name(vtable_func_offset, v_func_name, idaapi.SN_FORCE)
            idc.add_struc_member(struct_id, v_func_name, vtable_func_offset , FF_DWRD, -1, 4)
            vtable_func_offset += 4
            vtable_func_value = get_wide_dword(p_vtable_addr + vtable_func_offset)

        #FIXIT: idc.SetMemberComment(struct_id, """ + offset + """ , "Was called form address:" + str(hex(idc.GetRegValue("eip"))) , 1)
